...
QUALITY_SOURCE_CEILINGS = {
    ...
    "no_fly_sensitive": 0.82,
    "population": 0.82,
    "landuse": 0.78,
    "poi": 0.72,
    "transport_hydro": 0.80,
}
...
def _downsample_profile_samples(samples: List[Dict[str, float]], max_points: int = 260) -> List[Dict[str, float]]:
    ...
    keep_idx = set(range(0, len(samples), step))
    keep_idx.add(len(samples) - 1)
    fields = ("surface_msl_m", "route_alt_msl_m", "terrain_msl_m")

    # Preserve skipped points that protrude beyond the envelope formed by
    # adjacent retained anchors. This keeps narrow spikes visible without
    # restoring the full unsampled series.
    for idx in range(1, len(samples) - 1):
        if idx in keep_idx:
            continue
        left = idx - 1
        while left >= 0 and left not in keep_idx:
            left -= 1
        right = idx + 1
        while right < len(samples) and right not in keep_idx:
            right += 1
        if left < 0 or right >= len(samples):
            continue
        for field in fields:
            value = float(samples[idx].get(field, 0.0))
            left_value = float(samples[left].get(field, 0.0))
            right_value = float(samples[right].get(field, 0.0))
    ...
    return [samples[i] for i in sorted(keep_idx)]
...
def _extract_timestamp_from_path(path: Path) -> Optional[float]:
    if not path:
        return None
...
def _freshness_from_paths(paths: List[Path], run_ts: float, stale_days: float = QUALITY_STALE_DAYS, fallback: float = 0.65) -> float:
    values: List[float] = []
    for path in paths:
        if not path:
            continue
        ts = _extract_timestamp_from_path(path)
        if ts is None:
            continue
        age_days = max(0.0, (float(run_ts) - float(ts)) / 86400.0)
        values.append(_clip01(1.0 - (age_days / max(1.0, float(stale_days)))))
    if not values:
        return _clip01(fallback)
    return round(sum(values) / len(values), 4)


def _quality_layer_score(
    ceiling: float,
    spatial: float,
    freshness: float,
    attribute: float,
    ...
    components = {
        "source_ceiling": round(_clip01(ceiling), 4),
        "spatial": round(_clip01(spatial), 4),
        "freshness": round(_clip01(freshness), 4),
        "attribute": round(_clip01(attribute), 4),
        "route_support": round(_clip01(route_support), 4),
        "geometry": round(_clip01(geometry), 4),
    }
    raw = (
        0.35 * components["spatial"]
        + 0.20 * components["freshness"]
        + 0.20 * components["attribute"]
        + 0.15 * components["route_support"]
        + 0.10 * components["geometry"]
    )
    components["raw"] = round(raw, 4)
    components["score"] = round(components["source_ceiling"] * raw, 4)
    return components


def _line_substring(line: LineString, start_m: float, end_m: float) -> Optional[LineString]:
    try:
        seg = substring(line, max(0.0, float(start_m)), max(0.0, float(end_m)))
    except Exception:
        return None
    ...
    if seg.geom_type == "MultiLineString":
        parts = [part for part in seg.geoms if part.length > 0.0]
        if not parts:
            return None
        parts.sort(key=lambda item: item.length, reverse=True)
        return parts[0]
    if seg.geom_type == "Point":
        start_pt = line.interpolate(max(0.0, float(start_m)))
        end_pt = line.interpolate(max(0.0, float(end_m)))
        if start_pt.equals(end_pt):
            end_pt = line.interpolate(min(float(line.length), max(0.0, float(end_m)) + 1.0))
        return LineString([start_pt.coords[0], end_pt.coords[0]])
    return None


def _profile_samples_in_range(samples: List[Dict[str, float]], start_m: float, end_m: float) -> List[Dict[str, float]]:
    out = [s for s in samples if start_m <= _safe_float(s.get("distance_m", 0.0), 0.0) <= end_m]
    if out:
        return out
    nearest = []
    for sample in samples:
        dist_m = _safe_float(sample.get("distance_m", 0.0), 0.0)
        if abs(dist_m - start_m) <= ALT_PROFILE_SPACING_M or abs(dist_m - end_m) <= ALT_PROFILE_SPACING_M:
            nearest.append(sample)
    return nearest


def _quality_note(summary: Dict[str, Any]) -> List[str]:
    notes: List[str] = []
    spike_ratio = _safe_float(summary.get("spike_dependency_ratio", 0.0), 0.0)
    if spike_ratio >= 0.65:
        notes.append("依赖单点高障碍/高楼抬升航高")
    if _safe_float(summary.get("crowd_points_in_buffer", 0.0), 0.0) >= 4.0:
        notes.append("人群聚集 POI 命中较多")
    if _safe_float(summary.get("constraint_zone_hits", 0.0), 0.0) >= 1.0:
        notes.append("局部存在禁飞/学校等硬约束邻近")
    if _safe_float(summary.get("line_risk_overlap_m", 0.0), 0.0) >= 400.0:
        notes.append("线性风险叠覆较长")
    return notes
...
def build_data_quality_assessment(
    *,
    ...
    route_line_xy: LineString,
    profile_samples: List[Dict[str, float]],
    altitude_meta: Dict[str, Any],
    candidate: Dict[str, Any],
    candidate_results_count: int,
    nofly_counter: Dict[str, Any],
    school_counter: Dict[str, Any],
    line_risk_counter: Dict[str, Any],
    poi_risk_counter: Dict[str, Any],
    landuse_counter: Dict[str, Any],
    obstacle_counter: Dict[str, int],
    water_total_len_m: float,
    water_near_start_m: float,
    water_near_end_m: float,
    crowd_points_xy: List[Any],
    key_points_xy: List[Any],
    infra_geoms: List[Any],
    crowd_tree: Optional[STRtree],
    ...
    requested_min_turn_deg: float,
    selected_candidate_id: str,
) -> Dict[str, Any]:
    outputs = city_summary.get("outputs", {})
    poi_path = Path(outputs.get("poi", {}).get("geojson", "")) if outputs.get("poi", {}).get("geojson") else None
    landuse_path = Path(outputs.get("landuse", {}).get("geojson", "")) if outputs.get("landuse", {}).get("geojson") else None
    population_path = Path(outputs.get("population", {}).get("clipped_tif", "")) if outputs.get("population", {}).get("clipped_tif") else None
    roads_path = Path(outputs.get("transport", {}).get("roads_geojson", "")) if outputs.get("transport", {}).get("roads_geojson") else None
    hsr_path = Path(outputs.get("transport", {}).get("hsr_geojson", "")) if outputs.get("transport", {}).get("hsr_geojson") else None
    water_surface_path = Path(outputs.get("hydro", {}).get("water_surface_geojson", "")) if outputs.get("hydro", {}).get("water_surface_geojson") else None
    waterway_path = Path(outputs.get("hydro", {}).get("waterway_geojson", "")) if outputs.get("hydro", {}).get("waterway_geojson") else None
    nofly_path = DEFAULT_CIVIL_AIRPORT_NO_FLY_GEOJSON

    route_len_m = max(1.0, float(route_line_xy.length))
    route_len_km = route_len_m / 1000.0
    profile_count = len(profile_samples)
    building_hits = int(_safe_float(altitude_meta.get("building_hit_count", 0.0), 0.0))
    obstacle_hits = int(_safe_float(altitude_meta.get("obstacle_hit_count", 0.0), 0.0))
    min_surface_clearance = _safe_float(altitude_meta.get("min_surface_clearance_m", 0.0), 0.0)
    min_turn_angle = _safe_float(candidate.get("min_turn_angle_deg", requested_min_turn_deg), requested_min_turn_deg)
    buffer_metrics = candidate.get("buffer_metrics", {}) or {}
    pop_avg = _safe_float(candidate.get("path_population_mean", 0.0), 0.0)
    pop_p90 = _safe_float(candidate.get("path_population_p90", 0.0), 0.0)

    dem_spatial = profile_count / max(1.0, route_len_km * 25.0)
    dem_freshness = 0.75 if str(altitude_meta.get("terrain_source", "")) == "opentopodata" else _freshness_from_paths([dem_tif] if dem_tif else [], run_ts, fallback=0.72)
    dem_attr = 1.0 if profile_count > 0 else 0.0
    dem_support = 1.0 if profile_count > 0 else 0.0
    dem_geom = 0.90
    dem_layer = _quality_layer_score(
        QUALITY_SOURCE_CEILINGS["dem"] if str(altitude_meta.get("terrain_source", "")) == "opentopodata" else 0.95,
        dem_spatial,
        dem_freshness,
        dem_attr,
        dem_support,
    ...
    building_spatial = (obstacle_counter.get("building", 0) + obstacle_counter.get("obstacle", 0)) / 1500.0
    building_freshness = _freshness_from_paths([poi_path] if poi_path else [], run_ts, fallback=0.70)
    building_attr = 0.65 + 0.20 * _clip01((building_hits + obstacle_hits) / 20.0) + 0.15 * _clip01(_safe_float(buffer_metrics.get("high_building_overlap_m", 0.0), 0.0) / 100.0)
    building_support = (building_hits + obstacle_hits) / 20.0
    building_geom = 0.85
    building_layer = _quality_layer_score(
        QUALITY_SOURCE_CEILINGS["building_obstacle"],
        building_spatial,
        building_freshness,
        building_attr,
        building_support,
    ...
    school_total = int(_safe_float(school_counter.get("school", 0), 0.0) + _safe_float(school_counter.get("kindergarten", 0), 0.0))
    airport_city_match = 1.0 if int(_safe_float((nofly_counter.get("civil_dataset_city_match") or {}).get("dataset_features_matched_city", 0), 0.0)) > 0 else 0.0
    nofly_spatial = 0.40 * airport_city_match + 0.30 * _clip01((school_total + len(infra_geoms)) / 500.0) + 0.30 * _clip01(_safe_float(nofly_counter.get("civil_dataset", {}).get("dataset_features_loaded", 0), 0.0) / 200.0)
    nofly_freshness = _freshness_from_paths([nofly_path, poi_path] if poi_path else [nofly_path], run_ts, fallback=0.68)
    nofly_attr = 0.85
    nofly_support = max(
        _clip01(_safe_float(nofly_counter.get("edges_filtered_by_no_fly", 0), 0.0) / 250.0),
        _clip01(_safe_float(buffer_metrics.get("critical_infra_geoms_in_buffer", 0.0), 0.0) / 6.0),
        0.55 if airport_city_match > 0 else 0.35,
    )
    nofly_geom = 0.85
    nofly_layer = _quality_layer_score(
        QUALITY_SOURCE_CEILINGS["no_fly_sensitive"],
        nofly_spatial,
        nofly_freshness,
        nofly_attr,
        nofly_support,
    ...
    population_layer = _quality_layer_score(
        QUALITY_SOURCE_CEILINGS["population"],
        1.0 if population_path and population_path.exists() else 0.0,
        _freshness_from_paths([population_path] if population_path else [], run_ts, fallback=0.72),
        0.90 if population_path and population_path.exists() else 0.0,
        1.0 if pop_p90 > 0.0 or pop_avg > 0.0 else 0.0,
        0.90,
    )

    landuse_layer = _quality_layer_score(
        QUALITY_SOURCE_CEILINGS["landuse"],
        _safe_float(landuse_counter.get("total", 0.0), 0.0) / 2000.0,
        _freshness_from_paths([landuse_path] if landuse_path else [], run_ts, fallback=0.68),
        0.80 if landuse_path and landuse_path.exists() else 0.0,
        1.0 if _safe_float(landuse_counter.get("total", 0.0), 0.0) > 0 else 0.0,
        0.85,
    )

    poi_total = (
        _safe_float(poi_risk_counter.get("crowd", 0.0), 0.0)
        + _safe_float(poi_risk_counter.get("sensitive_facility", 0.0), 0.0)
        + _safe_float(poi_risk_counter.get("key_facility", 0.0), 0.0)
        + _safe_float(poi_risk_counter.get("crowd_from_high_rise", 0.0), 0.0)
    )
    poi_support = (
        _safe_float(buffer_metrics.get("crowd_points_in_buffer", 0.0), 0.0)
        + _safe_float(buffer_metrics.get("key_facility_points_in_buffer", 0.0), 0.0)
        + _safe_float(buffer_metrics.get("sensitive_facility_points_in_buffer", 0.0), 0.0)
    ) / 15.0
    poi_layer = _quality_layer_score(
        QUALITY_SOURCE_CEILINGS["poi"],
        poi_total / 1500.0,
        _freshness_from_paths([poi_path] if poi_path else [], run_ts, fallback=0.68),
        0.75 if poi_path and poi_path.exists() else 0.0,
        poi_support,
        0.90,
    )

    line_overlap_km = _safe_float(buffer_metrics.get("line_risk_overlap_m", 0.0), 0.0) / 1000.0
    transport_support = (
        0.50 * _clip01(line_overlap_km / 1.5)
        + 0.25 * _clip01(water_total_len_m / 5000.0)
        + 0.25 * _clip01((500.0 / max(500.0, water_near_start_m + water_near_end_m)) if math.isfinite(water_near_start_m + water_near_end_m) else 0.0)
    )
    transport_layer = _quality_layer_score(
        QUALITY_SOURCE_CEILINGS["transport_hydro"],
        0.45 * _clip01(_safe_float(line_risk_counter.get("highway", 0.0), 0.0) / 1500.0)
        + 0.20 * _clip01(_safe_float(line_risk_counter.get("hsr", 0.0), 0.0) / 100.0)
        + 0.15 * _clip01(_safe_float(line_risk_counter.get("high_voltage_power_line", 0.0), 0.0) / 30.0)
        + 0.20 * _clip01((water_total_len_m / 1000.0) / 6.0),
        _freshness_from_paths([roads_path, hsr_path, water_surface_path, waterway_path], run_ts, fallback=0.68),
        0.80 if roads_path and roads_path.exists() else 0.0,
        transport_support,
        0.85,
    )

    hard_constraint_index = round(
        0.30 * dem_layer["score"] + 0.45 * building_layer["score"] + 0.25 * nofly_layer["score"],
        4,
    )
    risk_assessment_index = round(
        0.35 * population_layer["score"]
        + 0.20 * landuse_layer["score"]
        + 0.25 * poi_layer["score"]
        + 0.20 * transport_layer["score"],
        4,
    )

    nofly_cross = 0.85 if airport_city_match > 0 else 0.45
    obstacle_cross = 0.55 if _safe_float(buffer_metrics.get("high_building_overlap_m", 0.0), 0.0) > 0 else 0.40
    poi_cross = 0.45
    population_cross = 0.45
    cross_source_agreement = round(
        0.40 * nofly_cross + 0.30 * obstacle_cross + 0.15 * poi_cross + 0.15 * population_cross,
        4,
    )

    candidate_diversity = _clip01(candidate_results_count / 3.0)
    perturbation_stability = 0.40
    turn_margin = _clip01((min_turn_angle - requested_min_turn_deg) / 30.0)
    clearance_margin = _clip01(min_surface_clearance / max(1.0, requested_clearance_m * 1.5))
    constraint_margin = round(0.5 * turn_margin + 0.5 * clearance_margin, 4)
    route_stability = round(0.40 * candidate_diversity + 0.35 * perturbation_stability + 0.25 * constraint_margin, 4)

    confidence_index = round(
        0.45 * hard_constraint_index
        + 0.20 * risk_assessment_index
        + 0.20 * cross_source_agreement
        + 0.15 * route_stability,
        4,
    )

    school_tree = STRtree(school_hard_zones_xy) if school_hard_zones_xy else None
    school_id_map = _build_geom_id_map(school_hard_zones_xy) if school_hard_zones_xy else {}
    segment_count = max(1, int(math.ceil(route_len_m / QUALITY_SEGMENT_LEN_M)))
    segments: List[Dict[str, Any]] = []
    perturbation_stability_local = 0.40
    for idx in range(segment_count):
        start_m = float(idx) * QUALITY_SEGMENT_LEN_M
        end_m = min(route_len_m, start_m + QUALITY_SEGMENT_LEN_M)
        seg_line = _line_substring(route_line_xy, start_m, end_m)
        if seg_line is None or seg_line.length <= 0.0:
            continue
        seg_len_m = max(1.0, float(seg_line.length))
        seg_buf = seg_line.buffer(ROUTE_BUFFER_M)
        seg_samples = _profile_samples_in_range(profile_samples, start_m, end_m)
        seg_sample_count = len(seg_samples)
        seg_building_hits = sum(1 for s in seg_samples if _safe_float(s.get("building_h_m", 0.0), 0.0) > 0.0)
        seg_obstacle_hits = sum(1 for s in seg_samples if _safe_float(s.get("obstacle_h_m", 0.0), 0.0) > 0.0)
        seg_max_building_h = max((_safe_float(s.get("building_h_m", 0.0), 0.0) for s in seg_samples), default=0.0)
        seg_max_obstacle_h = max((_safe_float(s.get("obstacle_h_m", 0.0), 0.0) for s in seg_samples), default=0.0)
        seg_min_clearance = min((_safe_float(s.get("surface_clearance_m", requested_clearance_m), requested_clearance_m) for s in seg_samples), default=requested_clearance_m)
        crowd_local = _count_geoms_intersecting(crowd_tree, seg_buf, crowd_points_xy, crowd_id_map)
        key_local = _count_geoms_intersecting(key_tree, seg_buf, key_points_xy, key_id_map)
        ...
        local_dem = _quality_layer_score(
            dem_layer["source_ceiling"],
            seg_sample_count / max(1.0, (seg_len_m / ALT_PROFILE_SPACING_M) * 0.9),
            dem_layer["freshness"],
            dem_layer["attribute"],
            1.0 if seg_sample_count > 0 else 0.0,
            dem_layer["geometry"],
        )
        local_building = _quality_layer_score(
            building_layer["source_ceiling"],
            0.55 * building_layer["spatial"] + 0.45 * _clip01((seg_building_hits + seg_obstacle_hits) / 6.0),
            building_layer["freshness"],
            0.60 + 0.20 * _clip01((seg_building_hits + seg_obstacle_hits) / 6.0) + 0.20 * _clip01(max(seg_max_building_h, seg_max_obstacle_h) / 80.0),
            max(0.45, _clip01((seg_building_hits + seg_obstacle_hits) / 4.0)),
            building_layer["geometry"],
        )
        local_nofly = _quality_layer_score(
            nofly_layer["source_ceiling"],
            0.45 * nofly_layer["spatial"] + 0.55 * _clip01((school_local + infra_local) / 6.0),
            nofly_layer["freshness"],
            nofly_layer["attribute"],
            max(0.40, _clip01((school_local + infra_local) / 5.0)),
            nofly_layer["geometry"],
        )
        local_population = _quality_layer_score(
            population_layer["source_ceiling"],
            population_layer["spatial"],
            population_layer["freshness"],
            population_layer["attribute"],
            1.0 if seg_pop_max > 0.0 else 0.0,
            population_layer["geometry"],
        )
        local_landuse = _quality_layer_score(
            landuse_layer["source_ceiling"],
            landuse_layer["spatial"],
            landuse_layer["freshness"],
            landuse_layer["attribute"],
            0.85,
            landuse_layer["geometry"],
        )
        local_poi = _quality_layer_score(
            poi_layer["source_ceiling"],
            poi_layer["spatial"],
            poi_layer["freshness"],
            poi_layer["attribute"],
            max(0.35, _clip01((crowd_local + key_local) / 6.0)),
            poi_layer["geometry"],
        )
        local_transport = _quality_layer_score(
            transport_layer["source_ceiling"],
            transport_layer["spatial"],
            transport_layer["freshness"],
            transport_layer["attribute"],
            max(0.35, _clip01(line_overlap_local / 350.0)),
            transport_layer["geometry"],
        )
        local_hard = round(0.30 * local_dem["score"] + 0.45 * local_building["score"] + 0.25 * local_nofly["score"], 4)
        local_risk = round(
            0.35 * local_population["score"] + 0.20 * local_landuse["score"] + 0.25 * local_poi["score"] + 0.20 * local_transport["score"],
            4,
        )
        spike_dependency = 0.0
        if max(seg_max_building_h, seg_max_obstacle_h) >= 60.0:
            spike_dependency = 1.0 if (seg_building_hits + seg_obstacle_hits) <= 1 else _clip01(2.0 / max(2.0, seg_building_hits + seg_obstacle_hits))
        local_obstacle_cross = 0.35 if spike_dependency >= 0.8 else 0.55 if seg_building_hits + seg_obstacle_hits > 0 else 0.45
        local_cross = round(0.40 * nofly_cross + 0.35 * local_obstacle_cross + 0.25 * (0.45 + 0.10 * _clip01((crowd_local + key_local) / 6.0)), 4)
        local_constraint_margin = round(0.55 * _clip01(seg_min_clearance / max(1.0, requested_clearance_m * 1.5)) + 0.45 * (1.0 - 0.60 * spike_dependency), 4)
        local_stability = round(0.40 * candidate_diversity + 0.35 * perturbation_stability_local + 0.25 * local_constraint_margin, 4)
        local_confidence = round(0.50 * local_hard + 0.20 * local_risk + 0.15 * local_cross + 0.15 * local_stability, 4)
        seg_summary = {
            "index": idx,
            "start_m": round(start_m, 2),
            "end_m": round(end_m, 2),
            "start_km": round(start_m / 1000.0, 3),
            "end_km": round(end_m / 1000.0, 3),
            "length_m": round(seg_len_m, 2),
            "hard_constraint_index": local_hard,
            "risk_assessment_index": local_risk,
            "confidence_index": local_confidence,
            "cross_source_agreement": local_cross,
            "route_stability": local_stability,
            "layer_scores": {
                "dem": local_dem["score"],
                "building_obstacle": local_building["score"],
                "no_fly_sensitive": local_nofly["score"],
                "population": local_population["score"],
                "landuse": local_landuse["score"],
                "poi": local_poi["score"],
                "transport_hydro": local_transport["score"],
            },
            "support": {
                "profile_samples": seg_sample_count,
                "building_hit_samples": seg_building_hits,
                "obstacle_hit_samples": seg_obstacle_hits,
                "max_building_h_m": round(seg_max_building_h, 2),
                "max_obstacle_h_m": round(seg_max_obstacle_h, 2),
                "min_surface_clearance_m": round(seg_min_clearance, 2),
                "crowd_points_in_buffer": int(crowd_local),
                "key_points_in_buffer": int(key_local),
                ...
                "spike_dependency_ratio": round(spike_dependency, 4),
            },
        }
        seg_summary["notes"] = _quality_note(seg_summary["support"])
        segments.append(seg_summary)

    def _pick_lowest(items: List[Dict[str, Any]], key: str) -> Dict[str, Any]:
        if not items:
            return {}
        chosen = min(items, key=lambda item: _safe_float(item.get(key, 1.0), 1.0))
        return {
            "index": int(chosen.get("index", 0)),
            "start_km": round(_safe_float(chosen.get("start_km", 0.0), 0.0), 3),
            "end_km": round(_safe_float(chosen.get("end_km", 0.0), 0.0), 3),
            "value": round(_safe_float(chosen.get(key, 0.0), 0.0), 4),
    ...
    overall = {
        ...
        "selected": bool(str(candidate.get("id", "")) == str(selected_candidate_id)),
        "profile_key": str(candidate.get("profile_key", "")),
        "route_km": round(route_len_km, 3),
        "hard_constraint_index": hard_constraint_index,
        "risk_assessment_index": risk_assessment_index,
        "confidence_index": confidence_index,
        "cross_source_agreement": cross_source_agreement,
        "route_stability": route_stability,
        "layer_scores": {
            "dem": dem_layer["score"],
            "building_obstacle": building_layer["score"],
            "no_fly_sensitive": nofly_layer["score"],
            "population": population_layer["score"],
            "landuse": landuse_layer["score"],
            "poi": poi_layer["score"],
            "transport_hydro": transport_layer["score"],
        },
        "layer_details": {
            "dem": dem_layer,
            "building_obstacle": building_layer,
            "no_fly_sensitive": nofly_layer,
            "population": population_layer,
            "landuse": landuse_layer,
            "poi": poi_layer,
            "transport_hydro": transport_layer,
        },
        "components": {
            "cross_source_agreement": cross_source_agreement,
            "route_stability": route_stability,
            "candidate_diversity": round(candidate_diversity, 4),
            "perturbation_stability_default": perturbation_stability,
            "constraint_margin": constraint_margin,
        },
        "assumptions": [
            "当前口径为工程产品口径，强调路线可用性而非监管认证。",
            "多源一致性与稳健性在未执行扰动重规划时按保守值计分。",
            "分段口径按 500m 切片，定位低覆盖/低置信路段。",
        ],
        "lowest_layer": min(
            [
                ("dem", dem_layer["score"]),
    ...
    return {
        ...
        "overall": overall,
        "segments": {
            "segment_length_m": QUALITY_SEGMENT_LEN_M,
            "count": len(segments),
            "lowest_hard_constraint_segment": _pick_lowest(segments, "hard_constraint_index"),
            "lowest_risk_segment": _pick_lowest(segments, "risk_assessment_index"),
            "lowest_confidence_segment": _pick_lowest(segments, "confidence_index"),
            "items": segments,
        },
    }


def _build_quality_summary_html(quality: Dict[str, Any]) -> str:
    overall = quality.get("overall", {}) or {}
    segments = (quality.get("segments", {}) or {}).get("items", []) or []
    if not overall:
        return ""
    low_hard = (quality.get("segments", {}) or {}).get("lowest_hard_constraint_segment", {}) or {}
    low_risk = (quality.get("segments", {}) or {}).get("lowest_risk_segment", {}) or {}
    low_conf = (quality.get("segments", {}) or {}).get("lowest_confidence_segment", {}) or {}
    top_segments = sorted(segments, key=lambda item: _safe_float(item.get("confidence_index", 1.0), 1.0))[:3]
    ...
    def _metric(label: str, value: Any) -> str:
        try:
            ...
            text = "--"
        return f'<div class="route-quality__metric"><span class="route-quality__metric-label">{html.escape(label)}</span><strong>{html.escape(text)}</strong></div>'
    ...
    for label, item in [
        ...
        if not item:
            continue
        notes = "；".join(str(x) for x in item.get("notes", [])[:2]) or "无显著异常"
        low_rows.append(
            f'<div class="route-quality__low-item"><strong>{html.escape(label)}</strong>'
            f'<span>{item.get("start_km", 0):.3f}-{item.get("end_km", 0):.3f} km</span>'
            f'<span>{item.get("value", 0):.3f}</span>'
            f'<span>{html.escape(notes)}</span></div>'
        )

    segment_rows = []
    for item in top_segments:
        notes = "；".join(str(x) for x in item.get("notes", [])[:2]) or "常规路段"
        segment_rows.append(
            f'<div class="route-quality__seg-item"><strong>{item.get("start_km", 0):.3f}-{item.get("end_km", 0):.3f} km</strong>'
            f'<span>H {item.get("hard_constraint_index", 0):.3f}</span>'
            f'<span>R {item.get("risk_assessment_index", 0):.3f}</span>'
            f'<span>C {item.get("confidence_index", 0):.3f}</span>'
            f'<span>{html.escape(notes)}</span></div>'
        )

    layer_scores = overall.get("layer_scores", {}) or {}
    layer_rank = sorted(layer_scores.items(), key=lambda item: _safe_float(item[1], 1.0))
    weak_layers = "".join(
        f'<span class="route-quality__tag">{html.escape(str(name))}: {float(score):.3f}</span>'
        for name, score in layer_rank[:3]
    )

    return f"""
      <div class="route-quality">
        <div class="route-quality__head">
          <div class="route-quality__title">数据覆盖率与结论置信度</div>
          <div class="route-quality__caption">工程产品口径 v1 | 分段长度 {(quality.get("segments", {}) or {}).get("segment_length_m", QUALITY_SEGMENT_LEN_M):.0f} m</div>
        </div>
        <div class="route-quality__metrics">
          {_metric("硬约束数据指数", overall.get("hard_constraint_index", 0.0))}
          {_metric("风险评估数据指数", overall.get("risk_assessment_index", 0.0))}
          {_metric("规划结论置信度", overall.get("confidence_index", 0.0))}
          {_metric("多源一致性", overall.get("cross_source_agreement", 0.0))}
          {_metric("结果稳健性", overall.get("route_stability", 0.0))}
        </div>
        <div class="route-quality__weak">{weak_layers}</div>
        <div class="route-quality__grid">
          <div class="route-quality__card">
            <div class="route-quality__card-title">最低分路段</div>
            {''.join(low_rows) or '<div class="route-quality__empty">无分段结果</div>'}
          </div>
          <div class="route-quality__card">
            <div class="route-quality__card-title">最低置信前三段</div>
            {''.join(segment_rows) or '<div class="route-quality__empty">无分段结果</div>'}
          </div>
        </div>
      </div>
"""
...
<style>
  ...
  .route-quality__title {
    font-size: 14px;
    font-weight: 700;
    color: #18314d;
  }

  .route-quality__caption {
    font-size: 12px;
    color: #5b7690;
  }

  .route-quality__metrics {
    display: grid;
  ...
  .route-quality__low-item,
  .route-quality__seg-item {
    display: grid;
    grid-template-columns: 104px 92px 54px 1fr;
    gap: 8px;
    align-items: start;
    font-size: 12px;
    color: #35506b;
  ...
  .route-quality__low-item:first-of-type,
  .route-quality__seg-item:first-of-type {
    border-top: none;
    padding-top: 0;
  }

  .route-quality__low-item strong,
  .route-quality__seg-item strong {
    color: #17324d;
  }

  .route-quality__empty {
    font-size: 12px;
    color: #607b96;
  }
...
def main() -> None:
    ...
    quality_variants_for_html: Dict[str, Dict[str, Any]] = {}
    ...
    run_ts = time.time()
    for cid, cand in result_by_id.items():
        quality_variants_for_html[cid] = build_data_quality_assessment(
            city_summary=summary,
            run_ts=run_ts,
            route_line_xy=cand["route_line_xy"],
            profile_samples=cand.get("altitude_profile_samples") or [],
            altitude_meta=cand.get("altitude_meta") or {},
            candidate=cand,
            candidate_results_count=len(candidate_results),
            nofly_counter=nofly_counter,
            school_counter=school_counter,
            line_risk_counter=line_risk_counter,
            poi_risk_counter=poi_risk_counter,
            landuse_counter=landuse_display_counter,
            obstacle_counter=obstacle_counter,
            water_total_len_m=water_total_len_m,
            water_near_start_m=water_near_start_m,
            water_near_end_m=water_near_end_m,
            crowd_points_xy=crowd_points_xy,
            key_points_xy=key_points_xy,
            infra_geoms=infra_geoms,
            crowd_tree=crowd_tree,
            ...
            requested_min_turn_deg=float(args.min_turn_angle_deg),
